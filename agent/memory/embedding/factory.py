...
def _init_explicit_provider(provider_key: str):
    ...
    try:
        provider_type = ""
        if resolved_provider_key == "custom":
        ...
        provider = create_embedding_provider(
            ...
            api_base=api_base,
            dimensions=dim,
            proxy=_resolve_proxy(provider_key),
        )
    except Exception as e:
        logger.error(
...
def _resolve_proxy(provider_key: str) -> Optional[str]:
    """Resolve the dedicated embedding proxy or inherit the matching model proxy."""
    from config import conf
    ...
    main_provider = _resolve_main_provider()
    if provider_key == main_provider:
        return conf().get("proxy") or None
    return None


def _resolve_main_provider() -> str:
    """Resolve the active chat provider for proxy inheritance."""
