...
<Tabs>
  <Tab title="CLI install (recommended)">
    ...
    To proxy only the browser runtime download:
    ```bash
    cow install-browser --proxy socks5h://127.0.0.1:1080
    ```
    This does not affect pip or npm.
  </Tab>
...
### Option 3: Camoufox backend

To replace the Playwright Chromium backend, use the daijro/camoufox Python Playwright-compatible backend. Install it with:

```bash
cow install-browser --engine camoufox
```
...
`proxy_default` controls whether the saved `camoufox.proxy` is applied at browser launch. When `false` (default), page traffic stays direct unless the agent passes `use_proxy: true` on a `browser` tool call — for example when navigating to sites blocked or unreachable from mainland China. This only enables the proxy already saved in config — arbitrary proxy URLs are not accepted at runtime.

`backend_proxy` (top-level under `tools.browser`) is separate: it proxies CowAgent's HTTP calls to the Camofox REST API, not Camoufox page traffic.

`camofox` is the existing Camofox REST backend, while `camoufox` is the Python package backend. Their configs are separate.
